...
def parse_args():
    ...
    parser.add_argument(
        "--train_fp",
        type=str,
        default="./data/triplets4testing.txt",
...
def get_gpt_response(client, content, temperature=0.5, max_tokens=300): 
    try: 
        ...
        print(f"An error occured with image/ word {content}")
        response = None
    return response
...
def process_triplet(
    ...
    image_query = get_image_query(image_triplet, prompt_images["prompt2"])
    word_query = get_word_query(word_triplet, prompt_words["prompt2"])
